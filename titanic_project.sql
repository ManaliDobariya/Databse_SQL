...
-- 1. Show the first 10 rows of the dataset with desc order.
...
select avg(age) as "Avg Age of Passenger" from titanic;
...
select Pclass, count(*) as " Total No. of passanger" from titanic group by Pclass order by Pclass asc;
...
select count(*) as "no of passanger" from titanic where Fare > (select avg(Fare) from titanic);
...
select 
sum(case when Sex = "male" then 1 else 0 end) as "Total Male",
sum(case when Sex = "female" then 1 else 0 end) as "Total Female" 
from titanic;
